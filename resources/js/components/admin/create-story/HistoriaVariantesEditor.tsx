...
interface HistoriaVariantesEditorProps {
    ...
    onUpdate: (
        index: number,
        field: keyof HistoriaVarianteForm,
...
export function HistoriaVariantesEditor({
    variantes,
    onAdd,
    onUpdate,
    onRemove,
}: HistoriaVariantesEditorProps) {
    return (
            ...
            {variantes.length > 0 ? (
                    ...
                    <table className="w-full border-collapse bg-[#F9FAFB] text-left">
                        <thead>
                            <tr className="border-b border-[#DFE4EA] bg-white">
                                <th className="min-w-[120px] px-3 py-2 text-[11px] font-bold tracking-wide text-[#7B7B7B] uppercase">
                                    Tipo
                                </th>
                                <th className="min-w-[220px] px-3 py-2 text-[11px] font-bold tracking-wide text-[#7B7B7B] uppercase">
                                    Valor
                                </th>
                                <th className="px-3 py-2 text-center text-[11px] font-bold tracking-wide text-[#7B7B7B] uppercase" />
                            </tr>
                        </thead>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) =>
                                                onUpdate(
                                                    ...
                                                    'tipo',
                                                    e.target
                                                        .value as HistoriaVarianteTipoForm,
                                                )
                                            }
                ...
                <div className="rounded-md border border-dashed border-[#DFE4EA] bg-[#F9FAFB] py-4 text-center">
                    <p className="text-[12px] text-[#A0A0A0]">
                        No hay variantes. Añade filas para describir papel o
                        color (texto libre en cada caso).
                    </p>
                </div>
            )}
        </div>
