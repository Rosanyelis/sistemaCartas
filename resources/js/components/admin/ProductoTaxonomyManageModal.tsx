...
export function ProductoTaxonomyManageModal({
        ...
        try {
            if (isCategoria) {
                const res = await laravelJsonFetch<Paginated<Row>>(
                    adminTaxonomiaUrls.productoCategorias.index({
                        page,
                        per_page: PER_PAGE,
                    }),
                );
                ...
                if (
                    !Number.isFinite(subCategoriaIdNum) ||
                    subCategoriaIdNum <= 0
                ) {
                    setRows([]);
                    setLastPage(1);
        ...
        if (!isCategoria) {
            setSubParentCategoryId(
                categoriaPadreId != null &&
                    Number.isFinite(categoriaPadreId) &&
                    categoriaPadreId > 0
        ...
        try {
            if (isCategoria) {
                await laravelJsonFetch(
                    adminTaxonomiaUrls.productoCategorias.store(),
                    {
                        method: 'POST',
                ...
                if (
                    ...
                    setError(
                        'Selecciona la categoría a la que pertenecerá la subcategoría.',
                    );
                    setSaving(false);
                    return;
                }
                await laravelJsonFetch(
                    adminTaxonomiaUrls.productoSubcategorias.store(),
                    {
                        method: 'POST',
                        body: JSON.stringify({
                            nombre: trimmed,
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            const path = isCategoria
                ...
                : adminTaxonomiaUrls.productoSubcategorias.destroy(
                      deleteTarget.id,
                  );
    ...
    const title = isCategoria
        ? 'Gestionar categorías'
        : 'Gestionar subcategorías';
    const currentPage = page;
    const last_page = lastPage;
    const emptyTableMsg = isCategoria
        ? 'No hay categorías registradas.'
        : 'No hay subcategorías para esta categoría.';
    const emptyTableMsgMobile = isCategoria
        ? 'No hay categorías.'
        : 'No hay subcategorías.';
    const deleteTitle = isCategoria
        ? 'Eliminar categoría'
                        ...
                        {!isCategoria ? (
                            <div>
                                ...
                                <p className="mt-1.5 text-[11px] text-[#6B7280]">
                                    El listado y las nuevas subcategorías se
                                    asocian a la categoría elegida.
                                </p>
                            </div>
                        ) : null}
