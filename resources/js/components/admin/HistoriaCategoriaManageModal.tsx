...
export function HistoriaCategoriaManageModal({ isOpen, onClose, onSaved }: HistoriaCategoriaManageModalProps) {
    ...
    const deleteBlockedTitle = 'No se puede eliminar: hay historias usando esta categoría';

    return createPortal(
        <>
            <div className="fixed inset-0 z-[80] flex items-center justify-center font-['Inter'] p-4">
                ...
                <div className="relative flex max-h-[90vh] w-[90%] flex-col overflow-hidden rounded-lg bg-white shadow-2xl md:w-[850px]">
                    <div className="flex shrink-0 items-center justify-between border-b border-[#F3F4F6] px-6 py-5">
                        <h3 className="text-[17px] font-semibold text-[#1B3D6D]">Gestionar categorías de historias</h3>
                    ...
                    <div className="shrink-0 space-y-4 border-b border-[#F3F4F6] px-6 py-4">
                        <div>
                            <label className="mb-2 block text-[13px] font-semibold text-[#1B3D6D]">Nueva categoría</label>
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="min-w-0 flex-1 rounded-[4px] border border-[#E5E7EB] bg-white px-3 py-2.5 text-[13.5px] text-[#1B3D6D] placeholder:text-[#9CA3AF] outline-none focus:border-[#1B3D6D] focus:ring-1 focus:ring-[#1B3D6D]"
                                />
                                <button
                                    type="button"
                                    disabled={saving}
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto">
                                                ...
                                                <tr key={r.id} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-3.5 text-center">
                                                        <button
                                                            ...
                                                            title={puedeEliminar ? 'Eliminar categoría' : deleteBlockedTitle}
                        ...
                        <div className="block divide-y divide-[#F3F4F6] md:hidden">
                            {loading ? (
                                ...
                                <p className="p-8 text-center text-[13px] text-[#7B7B7B]">No hay categorías.</p>
                            ) : (
                                rows.map((r) => {
                                    const count = r.historias_count ?? 0;
                                    const puedeEliminar = count === 0;
                                    return (
                                        <div key={r.id} className="flex items-center justify-between gap-3 px-4 py-3">
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="text-[12px] text-[#9CA3AF]">{count} historia(s)</p>
                                            </div>
                                            <button
                                                type="button"
                                                disabled={!puedeEliminar}
